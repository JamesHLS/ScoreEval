import music21.analysis.segmentByRests
import numpy as np
from music21 import *
import pandas as pd
import utils
...
def convert_mxml_to_csv(s, output_file_name, qlength=None, use_default_tempo=True):
    ...
    for partnum, part in enumerate(parts.parts):
        ...
        # group string, can play chords etc
                ...
                if partnum == 0:
                    ...
                    # Tempo texts
                        ...
                        if mm is not None:
                            ...
                            print(qstamp, n.measureNumber, n.content, mm)
                        continue
    ...
    def ts_to_timeoffset(bar_num):  # default tempo,
        """
        take time_sigs value from largest key below offset
        return time_offset
        quarternote bpm = ts.denominator * 30 / (ts.denominator/4)
        time_offset = offset * 60/quarternote bpm
        """
        ts = time_sigs.get(max(k for k in time_sigs.keys() if k <= bar_num))
        return bar_num * 60 / (ts.denominator * 30 / (ts.denominator / 4))
    if not tempos:
    ...
    bar_to_qstamp = lambda bar: df[df['bar'] == bar].iloc[0]['qstamp']
...
if __name__ == '__main__':
    filename = 'scores/Brahms_Symphony_1_third_movement.mxl'
